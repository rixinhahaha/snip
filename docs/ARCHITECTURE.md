...
| Image upscaling | Swin2SR (2x) / APISR (4x) | ONNX Runtime via Transformers.js |
...
src/
  main/                     # Main process (Node.js / CommonJS)
    ...
    upscaler/                # On-device image upscaling (2x/4x)
...
scripts/                     # Build and generation scripts
  download-models.js           # Download MiniLM + SlimSAM + upscaler models to vendor/models/
...
- **Xenova/4x_APISR_GRL_GAN_generator-onnx** (~4.8 MB q4) — 4x image upscaling
...
| `upscaleImage(data)` | R -> M | Upscale image via ONNX model (2x or 4x) in child process |
...
| HF models (MiniLM + SlimSAM + upscalers) | `vendor/models/` | `Resources/models/` |
